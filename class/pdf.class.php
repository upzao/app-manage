...
if(!class_exists('FPDF')) {
	...
	class PDF {
		...
		function AddPage($orientation='') {
			...
			$this->ColorFlag=$cf;
			if(!empty($this->bgpic)) {
				$this->Image($this->bgpic,0,0,210,297);
			}
...
//					$i+=2;
				...
				if ( $contentWidth + $cw > $maxWidth ) {
					...
					$contentWidth = 0;
					foreach ( $content as $k => $chunk ) {
						if($k > -1) $this->restoreFont( $font[ $k ] );
					...
					} else $this->_out( sprintf( '%.3f Tw', 0 ) );	// otherwise, we want normal spacing
					...
					$cutoffWidth = $contentWidth;
				} else {	// another character will fit, so add it on
					$contentWidth += $cw;
//					echo $i."<br>";
					if(ord($s{$i}) < 128) {
						$currContent .= $s{$i};
					} else {
						$currContent .= $i+1 < strlen($s) ? $s{$i}.$s{++$i} : $s{$i};
					}
		...
		//EDITEI
		/** 
		* Set Text Rendering Mode. This function is not being currently used
...
*******************************************************************************/
		...
		function _putinfo() {
			$this->_out('/Producer '.$this->_textstring('SupeSite/X-Space Comsenz Inc.'));
			if(!empty($this->title))
		...
		function _parsegif($file) {	//EDITEI - GIF support is now included
			...
			include_once(RELATIVE_PATH.'gif.php'); //GIF class in pure PHP from Yamasoft (http://www.yamasoft.com/php-gif.zip)
			
			$h=0;
			$bgColor = $w=0;
			$gif=new CGIF();
		
			if (!$gif->loadFile($file, 8920000))
