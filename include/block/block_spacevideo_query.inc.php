...
/*
	...
	$RCSfile: block_spacevideo_query.inc.php,v $
	$Revision: 1.8 $
	$Date: 2007/04/13 14:33:35 $
*/
...
if (empty($paramarr['sql'])) {
	...
	if(!empty($paramarr['showdetail']) && empty($paramarr['notype'])) {
		...
		$sql['join'] = 'LEFT JOIN '.tname('spacevideos').' ii ON ii.itemid=i.itemid';
	}
	...
	if(!empty($paramarr['itemid'])) {
		...
		if(!empty($paramarr['subtype'])) {
			$paramarr['subtype'] = getdotstring($paramarr['subtype'], 'char', false, array('media', 'real', 'flash'));
	...
	if(!empty($paramarr['order'])) {
		$paramarr['order'] = str_replace('a.', 'i.', $paramarr['order']);
...
if($listcount) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		//图片
		if(!empty($value['subtype'])) {//网络影音
			if(!empty($value['image'])) {
				$value['image'] = A_URL.'/'.$value['image'];
			} else {
				$value['image'] = S_URL.'/images/base/video_nothumb.jpg';
			}
		} else {
			if(!empty($value['image'])) {
				$value['image'] = S_URL.'/'.$value['image'];
			} else {
				$value['image'] = S_URL.'/images/base/novideo.jpg';//本地影音
			}
		}
