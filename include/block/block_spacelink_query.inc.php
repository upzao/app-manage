...
/*
	...
	$RCSfile: block_spacelink_query.inc.php,v $
	$Revision: 1.16 $
...
if(empty($paramarr['sql'])) {
	...
	if(!empty($paramarr['showdetail']) && empty($paramarr['notype'])) {
		...
		$sql['join'] .= ' LEFT JOIN '.tname('spacelinks').' ii ON ii.itemid=i.itemid';
	}
	if(!empty($paramarr['showspacename'])) {
		$sql['select'] .= ', u.spacename';
...
if($listcount) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		//链接
		if(!empty($value['url'])) $value['linkurl'] = $value['url'];//书签地址
		if($value['type'] == 'news') {
