...
/*
	...
	$RCSfile: block_spacegoods_query.inc.php,v $
	$Revision: 1.17 $
...
if(empty($paramarr['sql'])) {
	...
	if(!empty($paramarr['showdetail']) && empty($paramarr['notype'])) {
		...
		$sql['join'] .= ' LEFT JOIN '.tname('spacegoods').' ii ON ii.itemid=i.itemid';
	}
	...
	if(!empty($paramarr['itemid'])) {
		...
		if(!empty($paramarr['subtype'])) {
			$paramarr['subtype'] = getdotstring($paramarr['subtype'], 'char', false, array('goods', 'auction'));
